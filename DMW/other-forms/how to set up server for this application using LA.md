...
# Updated flow (RFQ → Abstract → Post-Qualification) with TWG Report-driven post-qualification steps, aligned to your SVP memo + actual RFQ/AOQ/TWG formats.

Updated RFQ → Abstract → Post-Qualification flow
A) RFQ issuance and receipt (BAC Secretariat)
Prepare RFQ (links to PR No., includes purpose, item specs, ABC, and T\&C).​
Send/serve RFQ to suppliers and accept quotations in duplicate or via email, stamped with date/time received by BAC Secretariat.​
Enforce RFQ rules on receipt: deadline is within 3 days from receipt and not beyond 8:00 AM of the last day; bids higher than ABC are automatically disqualified; PhilGEPS registration is required per RFQ T\&C.​
If ABC ≥ 200,000, do PhilGEPS posting for at least 3 calendar days (per SVP memo).​
B) Abstract of Quotations (BAC)
Open and record quotations and prepare the Abstract of Quotation (AOQ) within 1 calendar day after the deadline (per SVP memo).​
AOQ structure (as-is): shows AOQ No. + date, particulars with ABC, supplier comparison columns (unit/total or total), notes/conditions, purpose, certification text (“opened and recorded”), and recommendation to award to the most responsive quotation within ABC.​
Determine the preliminary winner = lowest calculated and responsive quotation based on AOQ results and remarks (e.g., “Disqualified. No attachment of documents”).​
C) Post-Qualification (BAC through TWG) — UPDATED using TWG Report format
Issue TWG post-qualification action for the proposed LCRB (and/or top-ranked supplier) and schedule verification.
TWG conducts post-qualification review (TWG Report explicitly states this is done “for the purpose of determining compliance and responsiveness” pursuant to IRR/RA 12009; it is a verification stage after quotations).​
Document verification against originals: TWG verifies submitted eligibility/supporting documents “against the original copies” and confirms they are true/faithful reproductions.​
Check legal/eligibility documents (as used in TWG table): Business/Mayor’s Permit, PhilGEPS Certificate of Registration, Omnibus Sworn Statement (may be marked NA), Tax Clearance (noted “during payment”), plus the quotation.​
Check technical compliance: TWG confirms the bidder complied with end-user technical specifications (TWG report includes a dedicated “Compliance to the Technical Requirements of the End-User” section).​
Confirm financial/price reasonableness by referencing the supplier quotation figures and ABC in the TWG report’s quotation summary table.​
TWG Findings \& Recommendation: TWG declares whether the bidder “has complied and is responsive with all Legal, Technical, Financial and Post-Qualification requirements” and recommends award to BAC.​
D) Decision gates after TWG (important system behavior)
If TWG = PASS → proceed to BAC Resolution recommending award (Step 6 in SVP memo: “Recommendation of Award – BAC Resolution”).
If TWG = FAIL (e.g., missing attachments/disqualified noted in AOQ) → mark bidder as non-responsive/disqualified, then run post-qualification on the next-ranked quotation (repeat Steps 8–14) until a responsive bidder is confirmed or procurement is declared failed/needs re-RFQ.
...
D --> F[Deadline reached]
E --> F[Deadline reached]

F --> G[BAC opens \& records quotations]
G --> H[Prepare AOQ/Abstract<br/>Supplier comparison, notes, certification, recommended award]
H --> I[Identify top-ranked / LCRB candidate<br/>based on AOQ, within ABC]
I --> J[TWG Post-Qualification Review]
J --> K[Verify documents vs originals<br/>Business Permit, PhilGEPS Reg, OSS (NA if applicable), etc.]
K --> L[Validate technical compliance vs end-user specs]
L --> M[Validate quotation/ABC alignment]
M --> N{TWG result?}
N -->|PASS| O[TWG Report: Findings \& Recommend award to LCRB]
O --> P[BAC Resolution recommending award]
N -->|FAIL| Q[Disqualify / mark non-responsive<br/>Proceed to next-ranked bidder]
Q --> J

update the new ui base on this flow of the system

Below is the **UI update (MD-ready)** to align your system with the updated flow **RFQ → Abstract (AOQ) → Post-Qualification (TWG Report-driven)**, including the decision gates and the exact RFQ/AOQ rules reflected in your provided formats.[^8_1][^8_2][^8_3]

## Updated flow: RFQ → AOQ → TWG Post-Qual

**Primary actors:** BAC Secretariat (RFQ receipt), BAC (AOQ + ranking), TWG (post-qualification verification + report), BAC (award recommendation).[^8_1]

**Non-negotiable RFQ receipt rules to enforce in UI:**

- Quotations may be received in duplicate copies or via email and must be **stamped with date/time received** by BAC Secretariat.[^8_2]
- Deadline rule: quotation must be received **within 3 days from receipt** and **not beyond 8:00 AM of the last day**.[^8_2]
- **Bids higher than ABC are automatically disqualified**, and suppliers **must be PhilGEPS-registered** per RFQ Terms \& Conditions.[^8_2]


## RFQ page (BAC Secretariat) — UI changes

Add/ensure these fields + behaviors on RFQ module:[^8_1]

- RFQ header fields: RFQ No., Date, linked PR No., Purpose, ABC, item specs, supplier invite list.[^8_1][^8_2]
- Receipt tracking table (NEW in RFQ UI): Supplier, Received via (email/physical), Received timestamp (required), “Stamped” indicator, Attachment(s) of submitted quotation.[^8_2]
- Validation rules (hard checks):
    - Block “Accept Quotation” if received timestamp is beyond the computed deadline window (3 days / 8:00 AM last day).[^8_2]
    - Auto-flag “Above ABC” quotations as **Disqualified (Above ABC)**.[^8_2]
    - Require a “PhilGEPS Registered” checkbox/field per supplier quote (or pull from Supplier master) because RFQ T\&C requires it.[^8_2]
- RFQ Terms \& Conditions should include and display “Term of Payment is Check ADA” (since this affects downstream “Paid ADA” status capture).[^8_2]


## Abstract of Quotation (AOQ) page (BAC) — UI changes

AOQ must remain **document-driven** and preserve the AOQ structure you’re using (AOQ No., date, particulars with ABC, supplier comparison columns, notes/conditions, purpose, certification text, and recommendation).[^8_3]

Add/ensure these AOQ behaviors:

- “Open \& record quotations” action creates the AOQ certification section (the AOQ format explicitly contains “opened and recorded…” + recommendation statement).[^8_3]
- Ranking engine (NEW):
    - Determine preliminary LCRB candidate from quotations and remarks; preserve disqualification remarks such as “Disqualified. No attachment of documents.”[^8_3]
    - Only allow “Recommended awardee” if within ABC and not disqualified.[^8_3]


## Post-Qualification page (TWG Report-driven) — UI changes

Your existing frontend spec already has Post-Qual fields (post-qual number, linked abstract, document checklist, TWG findings, uploads).[^8_1]
Update Post-Qualification to be explicitly **TWG Report-driven** with structured sections mirroring your TWG report format:[^8_1]

Required UI sections (recommended layout):

- **Candidate selection**
    - Auto-fill from AOQ: “Current candidate = Rank \#1 / LCRB”; button: “Move to next-ranked bidder” when FAIL.[^8_3][^8_1]
- **Document verification (vs originals)**
    - Checklist fields (table-style): Business/Mayor’s Permit, PhilGEPS Certificate/Registration, Omnibus Sworn Statement (allow “N/A”), Tax Clearance (support “during payment”), Supplier quotation, Other eligibility/supporting docs.[^8_1]
    - Per item: status (PASS/FAIL/NA), remarks, attachment upload (scanned copies), and “Verified against original” toggle.
- **Technical compliance**
    - Dedicated “Compliance to Technical Requirements of End-User” section with PASS/FAIL and remarks.[^8_1]
- **Financial/price validation**
    - Show ABC vs quoted price summary (pulled from AOQ) and mark if within ABC.[^8_3]
- **TWG findings \& recommendation**
    - Free-text findings + structured conclusion: PASS/FAIL, recommended to BAC (auto-enabled only if PASS).[^8_1]


## Decision gates \& looping behavior (critical)

Implement these gates exactly in UI logic:

- If TWG result = **PASS** → enable “Create BAC Resolution” from Post-Qualification.[^8_1]
- If TWG result = **FAIL** → mark bidder as non-responsive/disqualified in the case record, then allow TWG to run post-qualification for the **next-ranked** quotation (loop until a responsive bidder is confirmed or BAC declares failure / re-RFQ).[^8_3]
...
If you want this inserted into your “Latest UI” master document, tell me which file should be the final single source of truth (e.g., **FRONTEND_UI_LATEST_UPDATED.md**) and I’ll return a clean “drop-in replacement section” for that file.

<div align="center">⁂</div>

[^8_1]: FRONTEND_DOCUMENTATION.md

[^8_2]: RFQ-FEB-2026.xlsx

[^8_3]: ABS-JAN-2026-1.xlsx
